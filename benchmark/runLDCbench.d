...
struct Config
{
    string pattern = r".*\.d", dmd = "ldc2", dflags = "-mcpu=native -O3 -release", args;
...
void runTests(Config cfg)
{
    ...
    auto ldcself = buildPath(cwd, "runLDCbench.d");
    foreach(DirEntry src; dirEntries(cwd, "*.d", SpanMode.depth))
    {
        if (!src.isFile || src.name == self || src.name == ldcself || src.name.withExtension(".ignore").exists)
            continue;
...
void main()
{
    ...
    cfg.dmd = "ldc2"; //env.get("DMD", "ldc2");
    cfg.dmd ~= " ../source/swisstable/group.d ../source/swisstable/map.d ../source/swisstable/hash.d";
    if (cfg.compile)
